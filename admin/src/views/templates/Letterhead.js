...
const pdfStyles = StyleSheet.create({
  ...
  toLabel: { fontWeight: 'bold', fontSize: 10, marginBottom: 2, letterSpacing: 0.4, color: '#4B4B9E' },
  recipientLine: { marginBottom: 2 },
  datePill: { alignSelf: 'flex-end', border: '0pt solid #d7dde3', paddingVertical: 4, paddingHorizontal: 8, borderRadius: 6, marginTop: 5 },
  dateText: { textAlign: 'right', fontWeight: 'bold', fontSize: 9, color: '#4B4B9E' },
  ...
  sigName: { fontWeight: 'bold', fontSize: 11, color: '#4B4B9E' },
...
const LetterheadPdf = ({ data }) => {
  ...
  const bodyLines = (data.bodyLines || []).filter((line) => line.visible && line.value.trim() !== '').map((line) => line.value.trim())
        ...
        <Page size="A4" style={pdfStyles.page}>
          <View style={pdfStyles.headerContainer}>
            ...
            <View style={pdfStyles.contactSection}>
              {data.phone.visible ? <Text>TEL: {data.phone.value}</Text> : null}
              {data.website.visible ? <Text>WEB: {data.website.value}</Text> : null}
            </View>
          </View>
...
const defaultData = {
  ...
  brandName: { value: 'Company Name', visible: true },
...
export default function LetterheadDocument() {
  ...
  const normalizeBodyLines = (templateData) => {
    ...
    if (templateData?.description) {
      const plain = String(templateData.description).replace(/<[^>]+>/g, ' ').replace(/\s+/g, ' ').trim()
          ...
          <Grid item xs={12} md={5}>
            <Box sx={{ maxHeight: '85vh', overflowY: 'auto', pr: 1 }}>
              <Stack spacing={3}>
                ...
                <FieldToggle label="To Name" value={formData.recipientName.value} visible={formData.recipientName.visible} onChange={updateField('recipientName')}
                             onToggle={toggleField('recipientName')}/>
                <FieldToggle label="To Title" value={formData.recipientTitle.value} visible={formData.recipientTitle.visible} onChange={updateField('recipientTitle')}
                             onToggle={toggleField('recipientTitle')}/>
                <FieldToggle label="Company" value={formData.recipientCompany.value} visible={formData.recipientCompany.visible} onChange={updateField('recipientCompany')}
                             onToggle={toggleField('recipientCompany')}/>
                <FieldToggle label="Address" value={formData.recipientAddress.value} visible={formData.recipientAddress.visible} onChange={updateField('recipientAddress')}
                             onToggle={toggleField('recipientAddress')} multiline rows={2}/>
                ...
                <FieldToggle label="Sender Title" value={formData.senderTitle.value} visible={formData.senderTitle.visible} onChange={updateField('senderTitle')}
                             onToggle={toggleField('senderTitle')}/>
                ...
                <FieldToggle label="Footer Address" value={formData.footerAddress.value} visible={formData.footerAddress.visible} onChange={updateField('footerAddress')}
                             onToggle={toggleField('footerAddress')} multiline rows={2}/>
              </Stack>
            </Box>
          </Grid>
        </Grid>
